...
impl ClientEngine {
    ...
    pub async fn run(
        ...
        let mut ring_log_counter: u32 = 0;
            ...
            tokio::select! {
                ...
                _ = audio_tick.tick() => {
                    ...
                    // drains the ring during TX anyway).
                    {
                        ...
                        // Periodic ring buffer level logging (every ~1 second = 50 ticks)
                        ring_log_counter += 1;
                        if ring_log_counter >= 50 {
                            ring_log_counter = 0;
                            log::info!(
                                "ring={} jbuf={} jitter={:.1}ms target={} rtt={}ms loss={}%{}{}",
                                ring_level,
                                jitter_buf.depth(),
                                jitter_buf.jitter_ms(),
                                jitter_buf.target_depth(),
                                last_hb_ack_rtt,
                                current_loss_percent,
                                if ptt { " TX" } else { "" },
                                if skip_this_tick { " SKIP" } else if max_pull > 1 { " REFILL" } else { "" },
                            );
                        }
