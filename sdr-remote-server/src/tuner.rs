...
pub struct TunerStatus {
    /// 0=Idle, 1=Tuning, 2=DoneOk, 3=Timeout, 4=Aborted, 5=DoneAssumed
    pub state: u8,
    pub connected: bool,
    /// True if DONE_OK/DONE_ASSUMED but VFO moved >25kHz from tuned freq (set by network tick)
    pub stale: bool,
}
...
/// CTS never went TRUE after RTS LOW — tuner assumed already tuned.
/// Known JC-Control circuit limitation: brief LED pulses on already-tuned
/// frequencies are too short for the custom LED→CTS circuit to pass through.
pub const TUNER_DONE_ASSUMED: u8 = 5;

/// How long to wait for CTS TRUE before assuming "already tuned" (ms).
/// Empirically determined: successful CTS TRUE always arrives on the very first
/// read after RTS LOW (<5ms). 500ms provides ample margin for slower setups
/// while keeping the UI responsive. See PATCH-tuner-cts-fix.md for evidence.
const CTS_ASSUME_DEADLINE_MS: u64 = 500;
/// Duration of aggressive 5ms CTS polling after RTS LOW (ms).
const CTS_AGGRESSIVE_PHASE_MS: u64 = 200;
...
impl Jc4sTuner {
    ...
    /// `assume_tuned`: if true, treat "CTS never TRUE" as "already tuned" (DONE_ASSUMED)
    /// instead of TIMEOUT. Enable this for JC-Control boxes with limited LED→CTS circuits.
    pub fn new(
        port_name: &str,
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                tuner_thread(port, cmd_rx, status_for_thread, cat_tx, spe, rf2k, &port_name_owned, assume_tuned);
            })
    ...
    /// Reset DONE_OK / DONE_ASSUMED state back to IDLE (e.g. after band change)
    pub fn reset_done(&self) {
        let mut s = self.status.lock().unwrap();
        if s.state == TUNER_DONE_OK || s.state == TUNER_DONE_ASSUMED {
            s.state = TUNER_IDLE;
        }
    }
    ...
    loop {
        match cmd_rx.recv_timeout(Duration::from_millis(200)) {
            Ok(TunerCmd::StartTune) => {
                run_tune_sequence(&mut port, &cmd_rx, &status, &cat_tx, &spe, &rf2k, assume_tuned);
            }
...
fn run_tune_sequence(
    ...
    // After RTS LOW, CTS goes TRUE when the tuner starts, then FALSE when done.
    //
    // Known hardware limitation: the custom LED→CTS circuit in the JC-Control box
    // cannot pass through very brief LED pulses. When the tuner is already tuned,
    // the JC-4s gives a pulse too short for the circuit → CTS never goes TRUE.
    // In that case we treat it as "assumed already tuned" (TUNER_DONE_ASSUMED)
    // after CTS_ASSUME_DEADLINE_MS. See PATCH-tuner-cts-fix.md for full analysis.
    //
    // Polling strategy: 5ms for the first 200ms (defense-in-depth for circuits
    // with marginal timing), then 25ms until the deadline.
    let start = Instant::now();
    let timeout = Duration::from_secs(30);
    let assume_deadline = Duration::from_millis(CTS_ASSUME_DEADLINE_MS);
    let aggressive_phase = Duration::from_millis(CTS_AGGRESSIVE_PHASE_MS);
    let mut last_cts = port.read_clear_to_send().unwrap_or(false);
    info!("JC-4s: CTS = {} after RTS LOW", last_cts);

    // Wait for CTS TRUE (tuner started).
    if !last_cts {
        loop {
            ...
            if assume_tuned && start.elapsed() > assume_deadline {
                // CTS never went TRUE — JC-Control circuit did not pass the LED pulse.
                // Assume the tuner is already tuned for this frequency.
                info!("JC-4s: CTS stayed FALSE for {}ms — assuming already tuned (CTS-puls compensatie aan)",
                    CTS_ASSUME_DEADLINE_MS);
                let _ = cat_tx.blocking_send("ZZTU0;".to_string());
                info!("JC-4s: Tune carrier OFF (ZZTU0)");
                set_state(status, TUNER_DONE_ASSUMED);
                // No idle reset — stays visible until next tune (like DONE_OK)
                if restore_spe { safe_tune_operate(spe); }
                if restore_rf2k { safe_tune_operate_rf2k(rf2k); }
                return;
            }
            if !assume_tuned && start.elapsed() > Duration::from_secs(3) {
            ...
            // Aggressive 5ms polling for first 200ms, then 25ms
            if start.elapsed() < aggressive_phase {
