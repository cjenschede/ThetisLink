...
/// settle_ms=0 or absent: fast mode (SIGNAL_STRENGTH)
/// settle_ms>0: avg mode (AVG_SIGNAL_STRENGTH)
    ...
    if has_cli_args {
        ...
        rt.block_on(async {
            ...
            let (tuner_arc, cat_cmd_rx) = if let Some(ref port) = config.tuner_port {
                ...
                match tuner::Jc4sTuner::new(port, cat_tx, spe_arc.clone(), None) {
                    Ok(t) => {
...
pub async fn run_server_async(
    ...
    info!("Built with AI-assisted development");
    ...
    let rf2k = if rf2k_prebuilt.is_some() {
        ...
        Some(Arc::new(rf2k::Rf2k::new(addr, cli_cat_tx.clone())))
    } else {
        None
    };
    ...
    let (tuner, tuner_cat_rx) = if tuner_prebuilt.is_some() {
        ...
        match tuner::Jc4sTuner::new(port_name, cat_tx_for_tuner, spe_arc, rf2k_arc) {
            Ok(t) => {
