...
impl TciConnection {
    ...
    async fn send_init_commands(&mut self) {
        // Set Master volume to max (0 dB)
        self.send("VOLUME:0;").await;

        // Enable sensor push notifications (S-meter, TX power)
        self.send("RX_SENSORS_ENABLE:true,100;").await;
        self.send("TX_SENSORS_ENABLE:true,100;").await;

        // Configure and start audio streams
        ...
        // Start IQ streams (spectrum) — don't send IQ_SAMPLERATE, let Thetis
        // decide based on its Extended IQ Spectrum setting
        self.send("IQ_START:0;").await;
        self.send("IQ_START:1;").await;
        self.iq_started = true;

        // Request TX profile list
        self.send("tx_profiles_ex;").await;

        // Read current state of all controls (v2.10.3.13+)
        ...
        // Request calibration offsets (Thetis extension: display_offset for spectrum correction)
        self.send("calibration_ex:0;").await;
        self.send("calibration_ex:1;").await;
        // RX2 control readback
        ...
        // Request current VFO frequencies and mode (spectrum needs center freq at startup)
        self.send("VFO:0,0;").await;
        self.send("VFO:0,1;").await;
        self.send("VFO:1,0;").await;  // VFO-B via receiver 1
        self.send("MODULATION:0;").await;
        self.send("MODULATION:1;").await;
        self.send("DDS:0;").await;
        self.send("DDS:1;").await;

        // Query extended controls (capability-gated)
        ...
        if self.has_cap("ddc_sample_rate_ex") {
            info!("TCI: querying DDC sample rates");
            self.send("ddc_sample_rates_ex;").await;
            self.send("ddc_sample_rate_ex;").await; // queries both RX1 and RX2
        } else {
            info!("TCI: ddc_sample_rate_ex cap not available, skipping DDC rate query");
        }
        ...
        info!("TCI: init commands sent (audio 48kHz float32 mono, IQ rate from Thetis)");
    }
