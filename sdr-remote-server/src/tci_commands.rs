...
impl TciConnection {
    ...
    pub async fn set_tx_profile(&mut self, idx: u8) {
        if let Some(name) = self.tx_profile_names.get(idx as usize) {
            let cmd = format!("tx_profile_ex:{};", name);
            info!("TCI: set TX profile = \"{}\" (index {}) ({})", name, idx, cmd);
            self.send(&cmd).await;
        }
    }

    pub async fn set_nr(&mut self, level: u8) {
        // TCI: RX_NR_ENABLE_EX supports NR1-4 selection (v2.10.3.13+)
        if level == 0 {
    ...
    pub async fn set_nb(&mut self, level: u8) {
        // Without extensions: max NB1, level 2+ wraps to off
        let has_ext = self.has_cap("ctun_ex"); // ThetisLink extensions active
        let actual = if has_ext { level } else if level >= 2 { 0 } else { level };
        let enabled = actual > 0;
        let cmd = if has_ext && actual > 0 {
            format!("rx_nb_enable_ex:0,{},{};", if enabled { "true" } else { "false" }, actual)
        } else {
            format!("rx_nb_enable:0,{};", if enabled { "true" } else { "false" })
        };
        self.send(&cmd).await;
    ...
    pub async fn set_rx2_nb(&mut self, level: u8) {
        let has_ext = self.has_cap("ctun_ex");
        let actual = if has_ext { level } else if level >= 2 { 0 } else { level };
        let enabled = actual > 0;
        let cmd = if has_ext && actual > 0 {
            format!("rx_nb_enable_ex:1,{},{};", if enabled { "true" } else { "false" }, actual)
        } else {
            format!("rx_nb_enable:1,{};", if enabled { "true" } else { "false" })
        };
        self.send(&cmd).await;
    ...
    pub async fn send_spot(&mut self, callsign: &str, mode: &str, freq_hz: u64, color: u32, text: &str) {
        let cmd = format!("SPOT:{},{},{},{},{};", callsign, mode, freq_hz, color, text);
        self.send(&cmd).await;
    }
