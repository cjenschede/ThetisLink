...
impl SdrRemoteApp {
    ...
    fn sync_state(&mut self) {
        ...
        // Track tune frequency: on real tune (TUNING → DONE_OK) or first
        // DONE_OK after connect (tune_freq still 0). Ignores the fake
        // IDLE → DONE_OK transitions from the server's stale override.
        if state.tuner_state == 2 && (old_tuner_state == 1 || self.tuner_tune_freq == 0) {
    ...
    fn render_rx1_controls(&mut self, ui: &mut egui::Ui) {
        if self.popout_meter_analog {
            ...
            self.render_rx1_controls_inner(&mut measure);
            ...
            self.render_rx1_controls_inner(&mut left);
            ...
            self.render_rx1_controls_inner(ui);
        }
    }

    fn render_rx1_controls_inner(&mut self, ui: &mut egui::Ui) {
        let amber = Color32::from_rgb(255, 170, 40);

        // -- Top bar: frequency + mode --
        ui.horizontal(|ui| {
            // Inline frequency editing
            if self.freq_editing {
                ui.label(RichText::new("VFO A:").size(18.0).strong());
                let response = ui.add(
                    egui::TextEdit::singleline(&mut self.freq_edit_text)
                        .desired_width(140.0)
                        .font(egui::TextStyle::Heading),
                );
                if response.lost_focus() {
                    if ui.input(|i| i.key_pressed(egui::Key::Enter)) {
                        let clean: String = self.freq_edit_text.chars()
                            .filter(|c| c.is_ascii_digit())
                            .collect();
                        if let Ok(hz) = clean.parse::<u64>() {
                            if hz > 0 {
                ...
                let prefix = ui.add(
                    egui::Label::new(RichText::new("VFO A:  ").size(18.0).strong())
                        .sense(egui::Sense::click()),
                );
                if prefix.clicked() {
                    self.freq_editing = true;
                    self.freq_edit_text = if self.frequency_hz > 0 {
                        self.frequency_hz.to_string()
                    } else {
                        String::new()
                    };
                }
                if let Some(delta) = render_freq_scroll(ui, self.frequency_hz) {
                    let new_freq = (self.frequency_hz as i64 + delta).max(0) as u64;
        ...
        ui.horizontal(|ui| {
            ...
            let current_band = band_label(self.frequency_hz);
            for &(label, default_freq) in bands {
                let btn = if label == current_band {
                ...
                if ui.add(btn).clicked() {
                    ...
                    self.restore_band(Vfo::A, label, default_freq);
                    self.save_full_config();
                }
            }
        });
        ...
        ui.horizontal(|ui| {
            ...
            for &(mode_val, label) in modes {
                ...
                if ui.add(btn).clicked() {
                    let _ = self.cmd_tx.send(Command::SetMode(mode_val));
                    self.mode = mode_val;
        ...
        // -- Frequency step buttons --
        {
            ...
            let step_labels = ["10 Hz", "100 Hz", "500 Hz", "1 kHz", "10 kHz"];
            let mut freq_change: i64 = 0;

            ui.horizontal(|ui| {
                if ui.button(RichText::new(" - ").size(14.0)).clicked() {
                    freq_change = -(freq_steps[self.freq_step_index] as i64);
                }
                for (i, label) in step_labels.iter().enumerate() {
                    let btn = if i == self.freq_step_index {
            ...
            if freq_change != 0 {
                let new_freq = (self.frequency_hz as i64 + freq_change).max(0) as u64;
                let _ = self.cmd_tx.send(Command::SetFrequency(new_freq));
                self.set_pending_freq_a(new_freq);
            }
        }
    ...
    fn render_rx2_controls_with_split(&mut self, ui: &mut egui::Ui, show_split_button: bool, is_popout: bool) {
        if is_popout && self.popout_meter_analog {
            ...
            let measure_rect = egui::Rect::from_min_size(start, egui::vec2(total_w, 500.0));
            let mut measure = ui.new_child(egui::UiBuilder::new().max_rect(measure_rect).layout(egui::Layout::top_down(egui::Align::LEFT)));
            self.render_rx2_controls_inner(&mut measure, show_split_button, is_popout);
            let controls_h = measure.min_rect().height();
            ...
            let controls_rect = egui::Rect::from_min_size(start, egui::vec2(controls_w, 500.0));
            let mut left = ui.new_child(egui::UiBuilder::new().max_rect(controls_rect).layout(egui::Layout::top_down(egui::Align::LEFT)));
            self.render_rx2_controls_inner(&mut left, show_split_button, is_popout);
            ...
            self.render_rx2_controls_inner(ui, show_split_button, is_popout);
        }
    }

    fn render_rx2_controls_inner(&mut self, ui: &mut egui::Ui, show_split_button: bool, is_popout: bool) {
        let amber = Color32::from_rgb(255, 170, 40);

        // -- Top bar: frequency + mode --
        ui.horizontal(|ui| {
            if is_popout {
                ui.label(RichText::new("VFO B:  ").size(18.0).strong());
                if let Some(delta) = render_freq_scroll(ui, self.rx2_frequency_hz) {
                    let new_freq = (self.rx2_frequency_hz as i64 + delta).max(0) as u64;
                    let _ = self.cmd_tx.send(Command::SetFrequencyRx2(new_freq));
                    self.set_pending_freq_b(new_freq);
                }
            } else {
                let freq_text = if self.rx2_frequency_hz > 0 {
                    format!("VFO B:  {} Hz", format_frequency(self.rx2_frequency_hz))
                } else {
                    "VFO B:  --- Hz".to_string()
                };
                ui.label(RichText::new(freq_text).size(18.0).strong());
            }
        ...
        ui.horizontal(|ui| {
            ...
            let current_band = band_label(self.rx2_frequency_hz);
            for &(label, default_freq) in bands {
                let btn = if label == current_band {
                    egui::Button::new(RichText::new(label).size(11.0).strong())
                        .fill(Color32::from_rgb(100, 160, 230))
                ...
                if ui.add(btn).clicked() {
                    self.save_current_band(Vfo::B);
                    self.restore_band(Vfo::B, label, default_freq);
                    self.save_full_config();
                }
            }
        });
        ...
        ui.horizontal(|ui| {
            ...
            for &(mode_val, label) in modes {
                let btn = if self.rx2_mode == mode_val {
                    egui::Button::new(RichText::new(label).size(11.0).strong())
                        .fill(Color32::from_rgb(100, 160, 230))
                } else {
                    egui::Button::new(RichText::new(label).size(11.0))
                ...
                if ui.add(btn).clicked() {
                    let _ = self.cmd_tx.send(Command::SetModeRx2(mode_val));
                    self.rx2_mode = mode_val;
                }
            }
        });
        ...
        // -- Frequency step buttons --
        {
            ...
            ui.horizontal(|ui| {
                if ui.button(RichText::new(" - ").size(14.0)).clicked() {
                    freq_change = -(freq_steps[self.rx2_freq_step_index] as i64);
                }
                for (i, label) in step_labels.iter().enumerate() {
                    let btn = if i == self.rx2_freq_step_index {
                        egui::Button::new(RichText::new(*label).strong())
                            .fill(Color32::from_rgb(100, 160, 230))
                    } else {
                        egui::Button::new(*label)
            ...
            if freq_change != 0 {
                let new_freq = (self.rx2_frequency_hz as i64 + freq_change).max(0) as u64;
                let _ = self.cmd_tx.send(Command::SetFrequencyRx2(new_freq));
                self.set_pending_freq_b(new_freq);
            }
        }
    ...
    fn render_rx2_content(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        self.render_rx2_controls_with_split(ui, false, true);
        ui.separator();
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.separator();
            {
                ...
                if self.freq_editing {
                    ...
                    let freq_text = if self.frequency_hz > 0 {
                        format!("VFO A:  {} Hz", format_frequency(self.frequency_hz))
                    } else {
                        "VFO A:  --- Hz".to_string()
                    };
                    let response = ui.add(
                        egui::Label::new(RichText::new(freq_text).size(18.0).strong())
                            .sense(egui::Sense::click()),
                    );
                    if response.clicked() {
                        self.freq_editing = true;
                ...
                // Scroll wheel on the frequency area (only when spectrum is off —
                // when spectrum is on, scroll is handled by spectrum/waterfall displays)
                if !self.spectrum_enabled {
                    let scroll = ui.input(|i| i.raw_scroll_delta.y);
                    if scroll.abs() > 0.5 {
                        let steps = if scroll > 0.0 { 1i64 } else { -1i64 };
                        freq_change = steps * freq_steps[self.freq_step_index] as i64;
                    }
                }
            ...
            ui.horizontal(|ui| {
                ui.label("Mode:");
                let modes: &[(u8, &str)] = &[(0, "LSB"), (1, "USB"), (6, "AM"), (5, "FM")];
                for &(mode_val, label) in modes {
                    let btn = if self.mode == mode_val {
                        egui::Button::new(RichText::new(label).strong())
                            .fill(Color32::from_rgb(100, 160, 230))
                ...
                |ctx, _class| {
                    ...
                    egui::CentralPanel::default().show(ctx, |ui| {
                        ...
                        ui.columns(2, |cols| {
                            ...
                            self.render_rx1_controls(&mut cols[0]);

                            // Right column: RX2 controls with Split button on S-meter row
                            self.render_rx2_controls_with_split(&mut cols[1], true, true);
                        });
                    ...
                    if r1.is_positive() && r2.is_positive() {
                        ...
                        egui::Area::new(egui::Id::new("vfo_swap_joined"))
                            ...
                            .interactable(true)
                            .show(ctx, |ui| {
                                if ui.add(egui::Button::new(RichText::new("A<>B").size(10.0))).clicked() {
                        ...
                        egui::CentralPanel::default().show(ctx, |ui| {
                            ...
                            if show_rx2_popout {
                                ...
                                if r.is_positive() {
                                    ...
                                    if ui.put(btn_rect, egui::Button::new(RichText::new("A<>B").size(10.0))).clicked() {
                                        let _ = self.cmd_tx.send(Command::SetControl(ControlId::VfoSwap, 2));
                                    }
                                }
                            }
            ...
            egui::Window::new("About ThetisLink")
                ...
                .show(ctx, |ui| {
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ...
                        ui.add_space(6.0);
                        ui.label(RichText::new("Development").size(13.0).strong());
                        ui.label("Built with AI-assisted development.");
