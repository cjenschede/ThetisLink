# ThetisLink v0.4.1 — Installatiehandleiding
...
**TCI-modus (v0.4.0+):** Vanaf versie 0.4.0 maakt ThetisLink gebruik van het TCI WebSocket-protocol voor audio en spectrum. Dit betekent dat **VB-Audio Virtual Cable niet meer nodig is**. De server verbindt rechtstreeks met Thetis via TCI (poort 40001) en een aanvullende CAT-verbinding (poort 13013). Geen extra drivers, geen admin-rechten.
...
| ThetisLink-0.4.1.apk | Android client — telefoon/tablet |
...
1. Kopieer `ThetisLink-0.4.1.apk` naar je telefoon (USB, e-mail, of cloud)
...
```
adb install ThetisLink-0.4.1.apk
```
...
## Windows Remote Reboot instellen

Soms moet de server-PC op afstand herstart worden (bijv. na een Windows Update of een vastgelopen driver). Er zijn meerdere manieren om dit in te richten.

> **Tip:** ThetisLink Server kan Thetis automatisch opstarten via de Power-knop. Na een Windows reboot hoef je dus normaal alleen Windows zelf opnieuw op te starten — Thetis en de server kun je daarna remote bedienen.

### Optie A: Remote Desktop (RDP)

1. Open **Instellingen → Systeem → Extern bureaublad**
2. Schakel **Extern bureaublad inschakelen** aan
3. Firewall: poort **3389** (TCP) moet open staan
4. Verbind vanaf een andere PC met `mstsc` of een RDP-app op Android

### Optie B: PowerShell Remoting (WinRM)

Op de server-PC (eenmalig, als Administrator):
```powershell
...
Vanaf een andere PC:
```powershell
Restart-Computer -ComputerName <server-IP> -Force
```

Firewall: poort **5985** (HTTP) of **5986** (HTTPS) moet open staan.

### Optie C: Shutdown-commando

Vanaf een andere Windows-PC in hetzelfde netwerk:
```cmd
shutdown /r /m \\<computernaam> /t 0
```

### Firewall regels

| Methode | Poort | Protocol |
|---------|-------|----------|
| RDP | 3389 | TCP |
| WinRM (HTTP) | 5985 | TCP |
| WinRM (HTTPS) | 5986 | TCP |

Open de benodigde poort via: Windows Defender Firewall → Geavanceerde instellingen → Binnenkomende regel → Nieuwe regel → Poort.
