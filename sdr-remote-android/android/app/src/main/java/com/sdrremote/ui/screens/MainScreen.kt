...
fun MainScreen(viewModel: SdrViewModel = viewModel()) {
        ...
        AlertDialog(
            ...
            text = {
                Column(modifier = Modifier.fillMaxWidth().verticalScroll(rememberScrollState())) {
                    ...
                    Spacer(Modifier.height(6.dp))
                    Text("Development", fontWeight = FontWeight.Bold, fontSize = 13.sp)
                    Text("Built with AI-assisted development.", fontSize = 12.sp)
                    Spacer(Modifier.height(6.dp))
    ...
    // Record tune frequency on real tune (TUNING → DONE_OK) or first
    // DONE_OK after connect (tunerTuneFreq still 0). Ignores the fake
    // IDLE → DONE_OK transitions from the server's stale override.
    val tunerState = state.tunerState
    LaunchedEffect(tunerState) {
        if (tunerState == 2 && (lastTunerState == 1 || tunerTuneFreq == 0L)) {
